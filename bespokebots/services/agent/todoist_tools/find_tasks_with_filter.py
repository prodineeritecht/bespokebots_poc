...
class FindTasksWithFilterTool(TodoistBaseTool):
    ...
    description: str = """Use this tool to find tasks in Todoist using a filter. You can use any filter that you can use in Todoist. For example, to find all the tasks in the Personal project which are due today, you would set the filter parameter to 'today & #Personal'. Todoist follows a regular pattern for boolean symbols.  You also don't need a project_id to use the project in the filter, ust use the # symbol and then the project name. Additionally, sections can be specified with the / character.  For example, to find all 'InProgress' tasks in the Personal project, you would set the filter parameter to '#Personal & /InProgress'."""
    ...
    def _run(
        ...
        try:
            ...
            if tasks:
                ...
                return {"tasks": f"No tasks were found which match the filter: {filter}"}
